...
def is_kemono_url(url):
    """Kemono/Coomer/Nekohouse URL 감지"""
    try:
        # URL 정규화
        ...
        # kemono.su, coomer.su, nekohouse.su 등 정확한 도메인 매칭
        return "kemono" in netloc or "coomer" in netloc or "nekohouse" in netloc
    except:
        return False
...
def download_hitomi(url, folder_list):
    ...
    try:
        # 스크립트 디렉토리와 download 디렉토리 경로 설정
        script_dir = os.path.dirname(os.path.abspath(__file__))
        parent_dir = os.path.dirname(script_dir)
        download_dir = os.path.join(parent_dir, "download")
        hitomi_dir = os.path.join(download_dir, "hitomi")
        ...
        if os.path.exists(temp_folder):
            if not os.path.exists(hitomi_dir):
                os.makedirs(hitomi_dir)
            
            for folder_name in os.listdir(temp_folder):
                source_path = os.path.join(temp_folder, folder_name)
                if os.path.isdir(source_path):
                    dest_path = os.path.join(hitomi_dir, folder_name)
...
def download_arcalive(url, folder_list):
    ...
    try:
        ...
        parent_dir = os.path.dirname(script_dir)
        download_dir = os.path.join(parent_dir, "download")
        arcalive_dir = os.path.join(download_dir, "arcalive")
        ...
        # arcalive/채널명 폴더 생성
        channel_path = os.path.join(arcalive_dir, channel_name)
...
def download_twitter(url, browser="firefox"):
    ...
    try:
        ...
        parent_dir = os.path.dirname(script_dir)
        download_dir = os.path.join(parent_dir, "download")
        x_dir = os.path.join(download_dir, "x", username)
        ...
        print(f"[알림] 저장 위치: download/x/{username}/")
...
def main():
    ...
    # 상위 디렉토리에서 download_url.txt 경로 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(script_dir)
    url_file_path = os.path.join(parent_dir, "download_url.txt")
    ...
    if (kemono_urls or hanime_urls or youtube_urls or twitter_urls or general_urls) and not folder_list:
        print()
        if kemono_urls:
            print("=== kemono/coomer/nekohouse 다운로드 완료 ===")
            print("[알림] kemono/coomer/nekohouse 파일은 gallery-dl 기본 경로에 저장되었습니다")
        if hanime_urls:
            print("=== hanime 다운로드 완료 ===")
            print("[알림] hanime 파일은 download/hanime 폴더에 저장되었습니다")
        if youtube_urls:
            print("=== YouTube 다운로드 완료 ===")
            print("[알림] YouTube 파일은 download/youtube 폴더에 저장되었습니다")
        if twitter_urls:
            print("=== X/Twitter 다운로드 완료 ===")
            print("[알림] X/Twitter 파일은 download/x 폴더에 저장되었습니다")
        if general_urls:
